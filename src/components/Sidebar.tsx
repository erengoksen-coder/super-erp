...
// Lazy-load complex sub-components to prevent module initialization errors (reading 'call')
const CommandPalette = dynamic(() => import('@/components/dashboard/CommandPalette').then(mod => mod.CommandPalette), { ssr: false })
const NotificationDrawer = dynamic(() => import('@/components/dashboard/NotificationDrawer').then(mod => mod.NotificationDrawer), { ssr: false })
...
const GROUP_ORDER = ['', 'Üretim & Stok', 'Satış & Tedarik', 'Finans', 'Sistem']
...
// Use a lazy getter for menuGroups to ensure all icons and constants are fully loaded
let _menuGroups: { label?: string; items: MenuItem[] }[] | null = null
function getMenuGroups() {
  if (!_menuGroups) {
    _menuGroups = buildMenuGroups()
  }
  return _menuGroups
}
...
export default function Sidebar() {
  ...
  const roleLower = (user?.role || '').toString().trim().toLowerCase()
  const isBayiUser = roleLower === 'bayi'
  const isOperator = roleLower === 'operator'
  const permissions = user?.permissions ?? []
  
  // Professional Turkish Role Labels (Agi-OS Platinum Edition)
  const getRoleLabel = (role: string) => {
    switch (role?.toLowerCase()) {
      case 'admin': return 'Sistem Yöneticisi'
      case 'bayi': return 'Bayi Sorumlusu'
      case 'operator': return 'Üretim Operatörü'
      case 'planlama': return 'Üretim Planlama'
      case 'muhasebe': return 'Muhasebe & Finans'
      case 'depo': return 'Depo Sorumlusu'
      case 'sevkiyat': return 'Sevkiyat Sorumlusu'
      case 'satis': return 'Satış & Pazarlama'
      case 'satinalma': return 'Satın Alma'
      case 'kalite': return 'Kalite Kontrol'
      case 'management': return 'Üst Yönetim'
      case 'bakim': return 'Bakım Sorumlusu'
  ...
  const visibleMenuGroups = useMemo(() => {
    ...
    const filtered = filterMenuByPermissions(getMenuGroups(), permissions, isAdmin)
  ...
  // Global Zenith Hotkeys
  useHotkeys({ key: 'b', ctrl: true }, () => toggle())
  useHotkeys({ key: 'a', alt: true }, () => setIsNotificationsOpen(true)) // Use notifications as proxy or update state
  
  // Quick Nav: 1=Dashboard, 2=Production, 3=Inventory, 4=Orders, 5=Finance
  useHotkeys('1', () => router.push(ROUTES.HOME))
  useHotkeys('2', () => router.push(ROUTES.PRODUCTION))
  useHotkeys('3', () => router.push(ROUTES.INVENTORY))
  useHotkeys('4', () => router.push(ROUTES.ORDERS))
  useHotkeys('5', () => router.push(ROUTES.FINANCE))
      ...
      {visibleMenuGroups.map((group) => (
        <div key={group.label ?? 'main'} className={cn(group.label && 'mt-4')}>
          ...
          {group.items.map((item) => {
            ...
            const linkClass = cn(
              'w-full flex items-center gap-3 rounded-xl transition-all duration-300 relative group/link',
              collapsed ? 'justify-center px-0 py-3' : 'px-4 py-3 text-left',
              isActive
  ...
  return (
    <>
      ...
      >
        {/* Logo + collapse */}
        <div className="flex items-center justify-between h-20 px-4 border-b border-white/5 gap-2 bg-white/[0.02]">
          <Link
            href={ROUTES.HOME}
            className={cn(
          ...
          >
            <div className="shrink-0 flex items-center justify-center h-12 w-auto">
              {systemSettings?.company_logo ? (
                <img src={systemSettings.company_logo} alt="Logo" className="!h-10 !w-auto object-contain drop-shadow-glow" />
              ) : (
                <LogoWithBackground size="xs" className="!h-10 !w-auto !max-h-12 object-contain drop-shadow-glow" />
              )}
            </div>
            {!collapsed && (
              <div className="min-w-0 flex-1">
                <span className="block text-sm font-black text-white uppercase tracking-tighter leading-none">
                  {systemSettings?.company_name || 'ZENITH'}
                </span>
                <span className="block text-[8px] font-black text-primary uppercase tracking-[0.2em] mt-1">BUSINESS OS</span>
              </div>
            )}
          </Link>
        ...
        <div
          className={cn(
            'flex items-center gap-3 border-b border-white/5 bg-white/[0.01]',
            collapsed ? 'justify-center py-4 px-2' : 'p-4'
          )}
        >
          <div className="w-10 h-10 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center shrink-0 shadow-inner group hover:border-primary/30 transition-all">
            <User className="w-5 h-5 text-white/40 group-hover:text-primary" />
          </div>
          {!collapsed && (
            <div className="min-w-0 flex-1">
              ...
              <p className="text-[9px] font-bold text-white/20 uppercase tracking-widest mt-1.5 truncate">{getRoleLabel(user?.role || 'User')}</p>
            </div>
          )}
        </div>
