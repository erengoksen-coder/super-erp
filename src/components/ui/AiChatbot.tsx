...
export default function AiChatbot({ standalone = true, onClose }: { standalone?: boolean; onClose?: () => void }) {
            ...
            <AnimatePresence>
                {(standalone ? isOpen : true) && (
                    <motion.div
                        drag={standalone}
                        dragMomentum={false}
                        dragConstraints={{ left: typeof window !== 'undefined' ? -window.innerWidth + 400 : -1000, right: 0, top: typeof window !== 'undefined' ? -window.innerHeight + 600 : -1000, bottom: 0 }}
                        initial={{ opacity: 0, y: 100, scale: 0.8, filter: 'blur(10px)' }}
                        animate={{ opacity: 1, y: 0, scale: 1, filter: 'blur(0px)' }}
                        exit={{ opacity: 0, y: 100, scale: 0.8, filter: 'blur(10px)' }}
                        transition={{ type: 'spring', damping: 20, stiffness: 200 }}
                        className={cn(
                          "fixed z-50 w-[400px] h-[600px] max-h-[85vh] flex flex-col bg-slate-950/95 backdrop-blur-2xl border border-white/10 rounded-[2.5rem] shadow-[0_32px_64px_-16px_rgba(0,0,0,0.6)] overflow-hidden touch-none",
                          standalone ? "bottom-6 right-6" : "bottom-20 right-0"
                        )}
                    >
                        {/* Header */}
