...
export default function MessengerBox({ standalone = true, onClose }: { standalone?: boolean; onClose?: () => void }) {
  const pathname = usePathname()
  const user = useAuthStore((s) => s.user)
  const [open, setOpen] = useState(standalone ? false : true)
    ...
    <>
      {standalone && (
        <motion.button
          drag
          dragMomentum={false}
          dragConstraints={{ left: 0, right: typeof window !== 'undefined' ? window.innerWidth - 100 : 1000, top: typeof window !== 'undefined' ? -window.innerHeight + 100 : -1000, bottom: 0 }}
          type="button"
          onClick={() => {
            setOpen((o) => !o)
      ...
      <AnimatePresence>
        {(standalone ? open : true) && (
          <motion.div
            drag
            dragMomentum={false}
            dragConstraints={{ left: typeof window !== 'undefined' ? -window.innerWidth + 400 : -1000, right: typeof window !== 'undefined' ? window.innerWidth - 400 : 1000, top: typeof window !== 'undefined' ? -window.innerHeight + 440 : -1000, bottom: 0 }}
            initial={{ opacity: 0, y: 50, scale: 0.9 }}
            animate={{ opacity: 1, y: 0, scale: 1 }}
            exit={{ opacity: 0, y: 50, scale: 0.9 }}
            className={cn(
              "fixed z-[9999] flex w-[400px] max-w-[calc(100vw-3rem)] flex-col overflow-hidden rounded-2xl border border-slate-600/80 bg-slate-800/95 shadow-2xl backdrop-blur-sm touch-none",
              standalone ? "bottom-24 left-6" : "bottom-20 right-0"
            )}
