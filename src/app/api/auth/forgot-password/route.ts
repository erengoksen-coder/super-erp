...
// POST: Şifre sıfırlama linki gönder - Ironclad Transactional Version
export async function POST(request: NextRequest) {
  try {
    ...
    const user = db.prepare(`
      SELECT id, username, company_id, branch_id 
      FROM users 
      WHERE LOWER(email) = LOWER(?) AND deleted_at IS NULL AND is_approved = 1
    `).get(email) as { id: string; username: string; company_id: string; branch_id: string } | undefined

    if (!user) {
      // Güvenlik: E-posta yoksa bile "başarılı" gibi dönerek kullanıcı varlığını sızdırmıyoruz
      return ok({ message: 'E-posta adresi kayıtlı ise sıfırlama linki gönderilecektir.' })
    }

    const token = randomUUID()
    const expiresAt = new Date(Date.now() + TOKEN_VALID_HOURS * 60 * 60 * 1000).toISOString()

    // 2. Ironclad ATOMIC TRANSACTION for Token Creation
    ...
    AuditService.log({
      ...
      branchId: user.branch_id || DEFAULT_BRANCH_ID,
      actionType: 'UPDATE',
      entityName: 'users',
      entityId: user.id,
      description: `Şifre sıfırlama linki talep edildi: ${user.username}`,
    ...
    if (!result.ok) {
      return fail('E-posta gönderilemedi. Lütfen daha sonra tekrar deneyin.', { status: 500 })
    }
