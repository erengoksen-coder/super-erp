...
export const POST = withAuth(async (request: NextRequest, user: { userId: string; role: string }) => {
  ...
  // Audit Log & Telegram (Unified via AuditService)
  try {
    ...
    await AuditService.log({
      ...
      description: `📦 Bayi Siparişi: ${dealerName} tarafından ${orderCount} kalem ürün sipariş edildi: ${itemsText}`,
      ipAddress: request.headers.get('x-forwarded-for') || undefined,
      userAgent: request.headers.get('user-agent') || undefined
    })
  } catch (e) {
    console.warn('[bayi/orders] Audit/Telegram logging failed:', e)
  }
