...
export const GET = withAuth(async (request, user) => {
  return handleApi(async () => {
    ...
    const data = await getOrSetCache(cacheKey, 15_000, async () => {
      ...
      // 7. Phase 5 & 6 Integrated Stats
      const crmValueRow = db.prepare(`
        SELECT COALESCE(SUM(value), 0) as total 
        FROM crm_opportunities 
        WHERE stage != 'closed_lost' AND company_id = ?
      `).get(companyId) as { total: number }
      
      const invActivityRow = db.prepare(`
        SELECT COUNT(*) as count 
        FROM stock_movements 
        WHERE created_at >= datetime('now', '-24 hours') AND company_id = ?
      ...
      return {
        ...
        aging,
        crmPipelineValue: crmValueRow?.total || 0,
        recentInventoryMovements: invActivityRow?.count || 0,
