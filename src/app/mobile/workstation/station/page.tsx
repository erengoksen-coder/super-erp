...
export default function StationPage() {
  ...
  const [confirmComplete, setConfirmComplete] = useState<{ orderId: string; itemIndex?: number; itemTotal?: number; message: string } | null>(null)
  const [confirmRevert, setConfirmRevert] = useState<{ orderId: string; itemIndex?: number; itemTotal?: number; message: string } | null>(null)
  ...
  async function markAsCompleted(orderId: string, itemIndex?: number, itemTotal?: number) {
    ...
    setConfirmComplete({ orderId, itemIndex, itemTotal, message: confirmMessage })
  }

  async function executeMarkAsCompleted(orderId: string, itemIndex?: number, itemTotal?: number) {
  ...
  async function revertToPreviousStation(orderId: string, itemIndex?: number, itemTotal?: number) {
    ...
    setConfirmRevert({ orderId, itemIndex, itemTotal, message: confirmMessage })
  }

  async function executeRevertToPreviousStation(orderId: string, itemIndex?: number, itemTotal?: number) {
    ...
    try {
      const response = await fetch('/api/production/station/orders', {
        ...
        body: JSON.stringify({
          ...
          item_index: itemIndex,
          item_total: itemTotal,
          barcode: orders.find(o => o.id === orderId)?.barcode || null,
          serial_number: orders.find(o => o.id === orderId)?.serial_number || null,
          revert: true,
        }),
      })
  ...
  return (
    <div className="min-h-screen bg-gray-900 p-4">
      <ConfirmDialog
        ...
        onConfirm={() => confirmComplete && executeMarkAsCompleted(confirmComplete.orderId, confirmComplete.itemIndex, confirmComplete.itemTotal)}
      ...
      <ConfirmDialog
        ...
        onConfirm={() => confirmRevert && executeRevertToPreviousStation(confirmRevert.orderId, confirmRevert.itemIndex, confirmRevert.itemTotal)}
        title="İstasyona Geri Gönder"
        message={confirmRevert?.message || ''}
        variant="warning"
      />
