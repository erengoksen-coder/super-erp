...
export default function AccountsPage() {
  ...
  const stats = useMemo(() => {
    return {
      total: meta?.total || 0,
      customers: (meta as any)?.customerCount || 0,
      vendors: (meta as any)?.vendorCount || 0,
      balance: data?.reduce((sum: number, a: Account) => sum + (a.balance || 0), 0) || 0
    }
  }, [data, meta])
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      mutate()
      globalMutate((key: string) => key.startsWith('/api/accounts'))
    } catch (err: any) {
    ...
    >
      <div className="space-y-8 pb-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 animate-reveal">
           <ZenithCard glow className="group">
              <div className="flex items-center justify-between">
                 <div>
                    <p className="text-[10px] font-black text-white/40 uppercase tracking-widest mb-1">Toplam Cari</p>
                    <h3 className="text-3xl font-black text-white tracking-tighter">{meta?.total || 0}</h3>
                 </div>
                 <div className="p-4 bg-primary/20 rounded-[1.5rem] group-hover:scale-110 transition-all border border-primary/20">
                    <Users className="w-6 h-6 text-primary" />
                 </div>
              </div>
           </ZenithCard>
           
           <ZenithCard glow className="group" variant="outline">
              <div className="flex items-center justify-between">
                 <div>
                    <p className="text-[10px] font-black text-emerald-400 uppercase tracking-widest mb-1">Müşteriler</p>
                    <h3 className="text-3xl font-black text-white tracking-tighter">{stats.customers}</h3>
                 </div>
                 <div className="p-4 bg-emerald-500/10 rounded-[1.5rem] group-hover:scale-110 transition-all border border-emerald-500/20">
           ...
           <ZenithCard glow className="group" variant="outline">
              <div className="flex items-center justify-between">
                 <div>
                    <p className="text-[10px] font-black text-amber-400 uppercase tracking-widest mb-1">Tedarikçiler</p>
                    <h3 className="text-3xl font-black text-white tracking-tighter">{stats.vendors}</h3>
                 </div>
                 <div className="p-4 bg-amber-500/10 rounded-[1.5rem] group-hover:scale-110 transition-all border border-amber-500/20">
           ...
           <ZenithCard glow className="group">
              <div className="flex items-center justify-between">
                 <div>
                    <p className="text-[10px] font-black text-purple-400 uppercase tracking-widest mb-1">Net Bakiye</p>
                    <h3 className="text-2xl font-black text-white tracking-tight">
                       {stats.balance.toLocaleString('tr-TR')} <span className="text-sm font-normal text-white/30">₺</span>
                    </h3>
                 </div>
                 <div className="p-4 bg-purple-500/10 rounded-[1.5rem] group-hover:scale-110 transition-all border border-purple-500/20">
        ...
        {/* Toolbar & Search */}
        <ZenithCard className="animate-reveal" style={{ animationDelay: '100ms' }}>
               ...
               <Table>
                 ...
                 <TableBody>
                   {data.map((account) => (
                     <TableRow 
                        key={account.id} 
                        className="border-white/5 group hover:bg-white/5 transition-all cursor-pointer select-none"
