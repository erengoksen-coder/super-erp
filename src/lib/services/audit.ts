    ...
    try {
      ...
      db.prepare(`
        INSERT INTO audit_logs (
          ...
          entity_name, table_name, entity_id, old_data, new_data, 
      ...
      `).run(
        ...
        actionType,
        entityName || 'system',
        entityName || 'system', // table_name mapped from entityName
        entityId || null,
      ...
      // NEW: Telegram Zenith Alert Integration (Agi-OS Link)
      const isCritical = actionType === 'LOGIN' || 
                        actionType === 'DELETE' || 
                        (actionType === 'CREATE' && (entityName === 'users' || entityName === 'orders' || entityName === 'products' || entityName === 'production_orders')) ||
                        (description && (description.includes('kritik') || description.includes('iptal') || description.includes('hata')))

      if (isCritical) {
        const { TelegramService } = await import('@/lib/notifications/telegram')
        const icon = actionType === 'DELETE' ? '🗑️' : actionType === 'LOGIN' ? '🔐' : '✨'
        const title = `${icon} ${actionType}: ${entityName || 'Sistem Olayı'}`
        const details = description || `${entityName} üzerinde ${actionType} işlemi yapıldı.`
        
        let alertType: 'INFO' | 'URGENT' | 'SECURITY' = 'INFO'
        if (actionType === 'DELETE' || actionType === 'LOGIN') alertType = 'SECURITY'
        if (description?.toLowerCase().includes('kritik')) alertType = 'URGENT'

        TelegramService.sendAlert(title, details, alertType).catch(err => {
          console.warn('[AuditService] Telegram notification failed:', err.message)
        })
      }
