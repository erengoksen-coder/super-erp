...
export const inventoryService = {
  ...
  async getAllMaterials(companyId: string, branchId: string): Promise<MaterialWithStock[]> {
    ...
    const rows = db.prepare(`
      SELECT 
        m.id, m.code, m.name, m.category, m.unit, m.min_stock_level, m.max_stock_level, 
        m.unit_price, m.location, m.lead_time_days, m.last_purchase_date,
        ...
        COALESCE((SELECT SUM(quantity) FROM stock_reservations WHERE material_id = m.id AND status = 'active' AND deleted_at IS NULL), 0) as reserved_amount,
        COALESCE((
          SELECT SUM(b.quantity_required * po.quantity)
  ...
  /**
   * Toplu malzeme güncelleme (Excel Import Altyapısı)
   */
  async bulkUpdateMaterials(items: any[], companyId: string, branchId: string, userId: string) {
    const db = getDatabase()
    let updated = 0
    let created = 0

    const upsertStmt = db.prepare(`
      INSERT INTO materials (id, code, name, category, unit, min_stock_level, unit_price, location, company_id, branch_id)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
