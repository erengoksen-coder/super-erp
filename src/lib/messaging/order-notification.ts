...
function formatOrderMessage(orders: OrderItem[]): string {
  const lines = [
    ...
    ...orders.slice(0, 15).map((o) => {
      ...
      const dealerName = (o as any).dealer_name || o.customer_name || ''
      const dealer = dealerName ? `[Bayi: ${dealerName.replace(/</g, '&lt;').replace(/>/g, '&gt;')}] ` : ''
  ...
  try {
    ...
    if (telegramBotToken && telegramChatId) {
      ...
      for (const cid of chatIds) {
        const result = await sendTelegramMessage(telegramBotToken, cid, text)
        if (!result.ok) {
          console.error(`[Messaging] Telegram (${cid}):`, result.error)
        }
      }
    }
...
export async function sendOrderFailureNotification(payload: {
  dealer_name: string
  error: string
  product_name?: string
}): Promise<void> {
  try {
    ...
    const dealer = (payload.dealer_name || 'Bilinmeyen Bayi').replace(/</g, '&lt;').replace(/>/g, '&gt;')
    const errorMsg = (payload.error || 'Bilinmeyen hata').replace(/</g, '&lt;').replace(/>/g, '&gt;')
    const prod = (payload.product_name || '-').replace(/</g, '&lt;').replace(/>/g, '&gt;')

    const text = [
      '<b>❌ Sipariş Oluşturma Hatası</b>',
      '',
      `👤 <b>Bayi:</b> ${dealer}`,
      `📦 <b>Ürün:</b> ${prod}`,
      `⚠️ <b>Hata:</b> ${errorMsg}`,
      '',
      '<i>Sistem hatası nedeniyle sipariş kaydedilemedi.</i>'
    ].join('\n')

    const result = await sendTelegramMessage(telegramBotToken, telegramChatId, text)
    if (!result.ok) console.error('[Messaging] Telegram (order failure):', result.error)
  } catch (e) {
    console.error('[Messaging] sendOrderFailureNotification:', e)
  }
}
