...
export function logAudit(db: Database.Database, entry: AuditEntry) {
  try {
    ...
    const isCritical = entry.action === 'delete' || (entry.action === 'create' && (entry.tableName === 'orders' || entry.tableName === 'users' || entry.tableName === 'products'))
    if (isCritical) {
      import('@/lib/notifications/telegram').then(({ TelegramService }) => {
        const title = `${entry.action.toUpperCase()}: ${entry.tableName}`
        const details = `İşlem: ${entry.action}, Tablo: ${entry.tableName}, Kayıt: ${entry.recordId || 'Tümü'}`
        const alertType = entry.action === 'delete' ? 'SECURITY' : 'INFO'
        TelegramService.sendAlert(title, details, alertType).catch(() => {})
      }).catch(() => {})
    }
  } catch {
