...
export function getAccessTokenFromRequest(request: NextRequest) {
  ...
  const cookieValue = request.cookies && typeof request.cookies.get === 'function'
    ? request.cookies.get(AUTH_COOKIE)?.value
    : undefined

  return cookieValue || bearer || null
}

export function getRefreshTokenFromRequest(request: NextRequest) {
  if (!request || !request.cookies || typeof request.cookies.get !== 'function') return null
  return request.cookies.get(REFRESH_COOKIE)?.value || null
}

export async function getAuthUserPayload(request: NextRequest) {
