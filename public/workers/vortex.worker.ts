/**
 * Vortex Zero-Latency Worker (Platinum Edition)
 * 
 * Offloads 3D particle mathematics from the main UI thread.
 * Handles:
 * - 3D Perspective Projections
 * - Rotation Matrix Calculations
 * - Mouse Repulsion Physics
 * - Particle Drift and Momentum
 */
...
function init(pCount: number, sCount: number, w: number, h: number) {
  width = w;
  height = h;
...
function project(p: Point, rx: number, ry: number) {
  ...
  const screenX = x1 * scale + width / 2 + p.driftX;
  const screenY = y1 * scale + height / 2 + p.driftY;

  return { x: screenX, y: screenY, scale, z: z2 };
}

function update(mx: number, my: number, mrx: number, mry: number) {
  time += 0.005;
  rotationY += 0.0015;
  ...
  // Background Stars (Deep Parallax)
  const renderedStars = stars.map(s => {
    const proj = project(s, mrx * 0.05, mry * 0.05);
    return { x: proj.x, y: proj.y, scale: proj.scale, color: s.color, size: s.size, opacity: 0.3 };
  });

  // Main Sphere
  const renderedPoints = points.map(p => {
    const proj = project(p, rx, ry);
