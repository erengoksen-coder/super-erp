...
// Logo with background (dark grey rounded rectangle) - Uses image file from public folder
export function LogoWithBackground({ className = '', size = 'md' }: LogoProps) {
  const sizeClasses = {
    xs: 'h-12 w-auto',  // Küçük boyut (48px)
    sm: 'h-24 w-auto',  // 2x büyütüldü (48px -> 96px)
    md: 'h-32 w-auto',  // 2x büyütüldü (64px -> 128px)
    lg: 'h-48 w-auto',  // 2x büyütüldü (96px -> 192px)
    label: 'w-full h-auto',  // Etiket için tam genişlik
  }
