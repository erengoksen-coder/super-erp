  ...
  return (
    <>
      <motion.button
        drag
        dragMomentum={false}
        dragConstraints={{ left: 0, right: window?.innerWidth - 100, top: -window?.innerHeight + 100, bottom: 0 }}
        type="button"
        onClick={() => {
          setOpen((o) => !o)
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            drag
            dragMomentum={false}
            dragConstraints={{ left: 0, right: window?.innerWidth - 400, top: -window?.innerHeight + 440, bottom: 0 }}
            initial={{ opacity: 0, y: 50, scale: 0.9 }}
            animate={{ opacity: 1, y: 0, scale: 1 }}
            exit={{ opacity: 0, y: 50, scale: 0.9 }}
            className="fixed bottom-24 left-6 z-[9999] flex w-[400px] max-w-[calc(100vw-3rem)] flex-col overflow-hidden rounded-2xl border border-slate-600/80 bg-slate-800/95 shadow-2xl backdrop-blur-sm touch-none"
