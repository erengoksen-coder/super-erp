...
function PurchaseRequestsNotification() {
  ...
  async function loadPurchaseRequests() {
    try {
      const response = await fetch('/api/purchase-requests?status=draft')
      if (!response.ok) return
      const data = await response.json()
      setRequests(data.filter((r: any) => !dismissed.includes(r.id)))
    } catch (error) {
        ...
        return planning && (
          <div className="mt-6 bg-gray-900 rounded-lg border border-gray-800 p-6">
            ...
            <div className="space-y-4">
              {['iskelet', 'terzihane', 'döseme', 'montaj'].map((station) => {
