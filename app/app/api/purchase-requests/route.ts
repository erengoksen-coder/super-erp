...
export async function POST(request: NextRequest) {
  try {
    ...
    const { material_id, requested_quantity, unit_price, notes } = body
    ...
    try {
      db.prepare(`
        ...
        (id, request_number, material_id, requested_quantity, unit_price, total_amount, status, notes)
        VALUES (?, ?, ?, ?, ?, ?, 'draft', ?)
      `).run(id, requestNumber, material_id, requested_quantity, price, totalAmount, notes || null)
