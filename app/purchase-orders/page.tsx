...
export default function PurchaseOrdersPage() {
  const { data: ordersData, isLoading, mutate } = useApi<PurchaseOrder[]>('/api/purchase-orders')
  const { data: suppliersData } = useApi<Account[]>('/api/accounts?type=supplier')
  const { data: materialsData } = useApi<Material[]>('/api/materials')
  ...
  const [form, setForm] = useState({
    ...
    payment_terms_days: '',
    notes: '',
    items: [{ material_id: '', quantity: '', unit_price: '' }] as ItemForm[]
  })
    ...
    setForm((prev) => {
      const items = prev.items.map((item, itemIndex) => {
        ...
        if (field === 'material_id') {
          ...
          if (material && !nextItem.unit_price) {
            nextItem.unit_price = String(material.unit_price ?? '')
          }
        }
        return nextItem
      })
        ...
        <Table>
          <TableHeader>
            <TableRow className="border-gray-800">
              ...
              <TableHead className="h-8 px-4 py-2 text-xs">Tedarikçi</TableHead>
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50">
          <div className="bg-gray-900 rounded-lg border border-gray-800 p-6 w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-bold text-white">Yeni Satın Alma Siparişi</h2>
              <button
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-1">
                    Tedarikçi <span className="text-red-400">*</span>
                  </label>
                  <select
