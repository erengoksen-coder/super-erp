...
export const GET = withAuth(async () => {
    ...
    `).all(DEFAULT_COMPANY_ID, DEFAULT_BRANCH_ID) as any[]
        ...
        // Sayfa listesi
        const pages = [
            '/orders', '/invoices', '/shipments', '/accounts',
            '/inventory', '/production', '/reports', '/quality-control',
            '/warehouses', '/admin',
        ]
...
export const POST = withAuth(async (request) => {
    try {
        const db = getDatabase()
        const { userId, pagePath, canView, canCreate, canEdit, canDelete } = await request.json()
