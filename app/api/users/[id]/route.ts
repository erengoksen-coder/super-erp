...
export const PATCH = withAuth(async (
  ...
  return handleApi(async () => {
    ...
    // Kullanıcıyı bul
    const userData = db.prepare('SELECT * FROM users WHERE id = ? AND company_id = ? AND branch_id = ? AND deleted_at IS NULL')
      .get(userId, companyId, branchId) as any
    if (!userData) {
      return fail('Kullanıcı bulunamadı', { status: 404 })
    }
    ...
    db.transaction(() => {
      ...
      if (is_approved !== undefined || approved_by || full_name !== undefined || job_title !== undefined || role !== undefined || position !== undefined || email !== undefined || is_locked !== undefined || dealer_name !== undefined) {
        ...
        updateParams.push(userId, companyId, branchId)
      ...
      if (permissions && Array.isArray(permissions)) {
        db.prepare('DELETE FROM user_permissions WHERE user_id = ? AND company_id = ? AND branch_id = ?').run(userId, companyId, branchId)
      ...
      if (role !== undefined) {
        ...
        db.prepare('DELETE FROM user_roles WHERE user_id = ? AND company_id = ? AND branch_id = ?').run(userId, companyId, branchId)
        db.prepare(`
...
export const DELETE = withAuth(async (
  ...
  return handleApi(async () => {
    ...
    const userData = db.prepare('SELECT role, username, email, is_approved FROM users WHERE id = ? AND company_id = ? AND branch_id = ? AND deleted_at IS NULL')
      .get(userId, companyId, branchId) as any
