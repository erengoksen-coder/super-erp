...
export const GET = withAuth(async (
  ...
  return handleApi(async () => {
    ...
    const usersWithPermissions = users.map((user: any) => {
      ...
      `).all(user.id, companyId, branchId)
...
export const POST = withAuth(async (
  ...
  return handleApi(async () => {
    ...
    if (!username || !password) {
      return fail('Kullanıcı adı ve şifre gerekli', { status: 400 })
    }

    if (password.length < 6) {
      return fail('Şifre en az 6 karakter olmalıdır', { status: 400 })
    }
    ...
    }, { message: 'Kullanıcı başarıyla oluşturuldu', status: 201 })
  })
}, ['admin'])
