...
import { getDatabase, getSetting } from '@/lib/database/db'
...
export const POST = withAuth(async (request: NextRequest, user: { userId: string; role: string }) => {
  ...
  try {
    ...
    const events = getSetting(db, 'messaging_events') || 'order.created'

    if (token && chatId && events.includes('order.created')) {
      const orderCount = inserted.length
      const itemsText = inserted.map(o => `• ${o.product_name} (${o.quantity} adet)`).join('\n')
      const text = `<b>📦 Yeni Bayi Siparişi</b>\n\n` +
                 `<b>Bayi:</b> ${dealerName}\n` +
                 `<b>Kalem Sayısı:</b> ${orderCount}\n\n` +
                 `<b>Ürünler:</b>\n${itemsText}\n\n` +
                 `<pre>Sipariş No: ${inserted[0]?.order_number || '-'}</pre>`

      sendTelegramMessage(token, chatId, text).catch(err => {
        console.error('[bayi/orders] Telegram gönderim hatası:', err)
      })
    }
  } catch (e) {
    console.warn('[bayi/orders] Telegram bildirimi tetiklenemedi:', e)
  }
